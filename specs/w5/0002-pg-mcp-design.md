...
class DatabaseConfig(BaseModel):
    ...
    username: str
    password: SecretStr
    role: str | None = Field(default=None, description="可选的只读角色，若提供则在会话中执行 SET ROLE")
...
### 4.3 函数白名单守卫 (security/function_guard.py)
...
class FunctionGuard:
    ...
    DEFAULT_SAFE_FUNCS = {
        ...
        "date_trunc", "extract", "to_char", "to_timestamp",
...
class SchemaService:
    ...
    async def load_all(
        ...
        async with self._lock:
            cached = self._load_from_disk(db_name) if self.cache_config.enable_disk_cache else None
            if cached and self._is_cache_valid(cached):
                self._cache[db_name] = cached
                # 异步刷新，防止冷启动阻塞
                asyncio.create_task(self._refresh_in_background(db_name, schemas, exclude_tables))
                return cached
    ...
    async def _load_from_db(
        ...
        async with self.db_pool.acquire_readonly(db_name) as conn:
            ...
            for schema_name in schemas:
                schema_info = await self._load_schema(conn, schema_name, exclude_tables or [])
    ...
    async def _load_schema(
        ...
        for table_row in tables:
            if any(self._match_pattern(table_row["table_name"], p) for p in exclude_tables):
                continue
...
> 定时自动刷新：若 `cache.auto_refresh_interval_hours > 0`，应用启动后可基于 `asyncio.create_task` 启动循环任务，周期性串行调用 `load_all` 以满足 F-010/F-010a（需要记录刷新状态、失败回退与并发限流）。
...
class QueryService:
    ...
    async def _validate_result(
        ...
        if self.validation_service:
            validation = await self.validation_service.validate(user_query, sql, result)
        else:
            # 兼容直接调用模式
            summary = self.sanitizer.generate_summary(
                result.columns, result.rows, result.row_count
            )
            validation = await self.llm_client.validate_result(user_query, sql, summary)
...
### 6.3 结果验证服务 (services/validation_service.py)
...
class ValidationService:
    ...
    def __init__(self, sanitizer: Sanitizer, llm_client: LLMClient, sample_rows: int, sample_cols: int):
        self.sanitizer = sanitizer
        self.llm_client = llm_client
        self.sample_rows = sample_rows
        self.sample_cols = sample_cols

    async def validate(self, user_query: str, sql: str, result: QueryResultData) -> dict:
        """返回验证结果和说明，不阻断主流程"""
        ...
        summary = self.sanitizer.generate_summary(result.columns, result.rows, result.row_count)
...
async def lifespan():
    ...
    query_service = QueryService(
        settings, db_pool, schema_service, llm_client, rate_limiter, validation_service
    )
